...
#define UNRELAXED 65535 //"nekonecno" (USHRT_MAX)
...
int *zachran_princezne(char **mapa, int n, int m, int t, int *dlzka_cesty)
{
    ...
    printf("\n");
    print_bin32(state, 0);
    printf("pos drak x:%d  y:%d\npos p1 x:%d  y:%d\npos p2 x:%d  y:%d\n", dragon_pos.x, dragon_pos.y, princess_pos[0].x, princess_pos[0].y, princess_pos[1].x, princess_pos[1].y);
    printf("DRAK %d %d _________________________________________________________\n", get_x(state), get_y(state));
...
MAP_I **dijkstra(char **mapa, MAP_I **map_info, uint32_t state, int height, int width, int **shortest_path, int *path_index)
{
    ...
    while (1)
    {
        ...
        //look right
        if (((y + 1) < width) && ((count == 1) || ((y + 1) != map_info[x][y].y_p))
        ...
        //look down
        if (((x + 1) < height) && ((count == 1) || ((x + 1) != map_info[x][y].x_p))
        ...
        if (get_D(state) == 1)
        {
            ...
            print_map_info(map_info, height, width);
            printf("\n\n");
        }
